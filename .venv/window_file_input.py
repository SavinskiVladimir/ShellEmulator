...
# метод для обработки выходного сообщения
def log_and_display_output(output):
    output_text.configure(state='normal')
    if output != '':
        output_text.insert(tk.END, f"{output}\n") # вывод сообщения в GUI
    ...
    try:
        with TarFile(tarfile_path, 'r') as files:
            for command in commands:
                ...
                output_text.configure(state='disabled')
                if command[:-1] == 'ls': # обработка команды ls
                ...
                elif command[:-1] == 'whoami': # обработка команды whoami
                    output = username
                ...
                elif command.startswith("exit"): # обработка команды exit
                    window.quit()
                    return
                elif command[:-1] == "history": # обработка команды history
                    try:
